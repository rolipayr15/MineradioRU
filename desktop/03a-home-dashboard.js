...
var HOME_DASHBOARD_REVIEW_DEFAULTS = [
  ...
  { text: 'Даже если ты упустил закат, впереди тебя ждёт целое небо звёзд.', source: 'Популярные отзывы' },
  { text: 'Сохраняй страсть к жизни и смело иди навстречу новым горизонтам.', source: 'Популярные отзывы' },
  { text: 'Все ответы — в пути, вся свобода — в ветре.', source: 'Популярные отзывы' },
  { text: 'Пусть звуки этого дня начнутся там, где тебе хорошо.', source: 'Mineradio' },
];
...
async function homeDashboardAttachVideo() {
  ...
  try {
    ...
    video.addEventListener('error', function () {
      ...
      homeDashboardReleaseVideoSource(true);
      homeDashboardNotify('这个 MP4 无法解码，请Изменить成 H.264 编码的 MP4');
    }, { once: true });
    ...
    homeDashboardReleaseVideoSource(true);
    homeDashboardNotify('主页 MP4 读取失败，请重新Выбрать');
  } finally {
...
async function handleHomeDashboardVideoFile(file) {
  if (!homeDashboardIsMp4File(file)) {
    homeDashboardNotify('这里只能Выбрать .mp4 文件');
    return;
  }
...
function renderHomeDashboardHero() {
  ...
  if (!hero.querySelector('.daily-review-card')) {
    hero.innerHTML = '<div class="daily-review-card">' +
      ...
      '<button type="button" onclick="homeDashboardNextReview()">Изменить一条</button>' +
      '<button id="home-dashboard-video-choose" type="button" onclick="openHomeDashboardVideoPicker()">Выбрать MP4</button>' +
      '<button id="home-dashboard-video-clear" type="button" onclick="clearHomeDashboardVideo()" hidden>移除视频</button>' +
      '<button type="button" onclick="openHomePlayerConsole()">Развернуть панель управления</button>' +
      '</div></div>' +
...
function renderHomeDashboardQuickCards() {
  ...
  var cards = [
    ...
    },
    {
      label: 'LIBRARY',
      title: 'Медиатека',
      sub: libraryCount ? (libraryCount + ' Обьектов · Локальная медиатека') : 'Плейлисты, локальная музыка и подключённые платформы',
    ...
    {
      label: 'RECENT',
      title: 'Недавно воспроизведённые',
      sub: recent ? ((recent.name || '最近一трека') + (recent.artist ? ' · ' + recent.artist : '')) : '播放过的歌曲会出现在这里',
...
function renderHomeDashboardDiscovery() {
  ...
  if (!homeDashboardDiscoveryCache.length) {
    root.innerHTML = '<button class="home-discovery-empty" type="button" onclick="openHomeDashboardLibrary()">' +
      '<strong>等待你的音乐</strong><span>登录平台或导入Локально音乐后生成推荐</span></button>';
    return;
  }
...
function playHomeDashboardDiscoverySong(index) {
  ...
  Promise.resolve(playQueueAt(currentIdx, {
    manual: true,
    context: { type: 'home-discovery', playlistName: 'Для тебя' },
...
function renderHomeInsightDock() {
  ...
  if (sub) sub.textContent = song ? (homeDashboardSubtitle(song) || '点击播放') : '点击打开Медиатека';
  var coverUrl = homeDashboardSongCover(song, 220);
  if (cover) homeDashboardSetStableBackgroundImage(cover, coverUrl);
  if (card) card.setAttribute('aria-label', song ? ('播放下一трека：' + (song.name || song.title || '未知歌曲')) : '打开Медиатека');
...
function playHomeNextFromDock() {
  ...
  if (next.song && next.queued) {
    ...
    Promise.resolve(playQueueAt(next.index, {
      manual: true,
      context: { type: 'home-next', playlistName: 'Очередь воспроизведения' },
  ...
  if (next.song) {
    ...
    Promise.resolve(playQueueAt(0, {
      manual: true,
      context: { type: 'home-next', playlistName: 'трека页推荐' },
...
function homePlatformRecommendationCard(kind, index, item, label) {
  ...
  var sub = '';
  if (kind === 'netease-playlist') sub = (item.trackCount ? item.trackCount + ' трека' : '推荐歌单') + (item.playCount ? ' · ' + compactHomeCount(item.playCount) + ' 播放' : '');
...
function renderHomePlatformDailyWindow(force) {
  ...
  for (var index = range.start; index < range.end; index += 1) {
    html.push(homePlatformRecommendationCard('netease-song', index, songs[index], '网易云Ежедневные рекомендации'));
  }
  html.push(homePlatformRecommendationSpacer(range.bottomRows, 'bottom'));
  grid.innerHTML = html.join('');
  grid.setAttribute('data-render-window', signature);
  grid.setAttribute('aria-label', '全部Ежедневные рекомендации，共 ' + songs.length + ' трека');
...
function homePlatformRecommendationEmptyHtml(source, message) {
  return '<div class="home-platform-recommend-empty"><strong>' + escHtml(homePlatformRecommendationSourceLabel(source)) + ' 暂无可用推荐</strong>' +
    '<span>' + escHtml(message || '当前版本没有可验证的Выбор платформ接口，未使用关键Текст搜索替代。') + '</span></div>';
}
...
function renderHomePlatformRecommendations() {
  ...
  if (source === 'netease') {
    if (homeDiscoverState.loading || homePlatformRecommendationState.neteaseLoading) {
      status.textContent = '正在读取网易云Выбор платформ…';
    ...
    if (songs.length) {
      sections.push('<section><h3>Ежедневные рекомендации<span id="home-platform-daily-count"></span></h3>' +
        '<div id="home-platform-daily-grid" class="home-platform-recommend-grid" role="list" aria-label="全部Ежедневные рекомендации"></div></section>');
    }
    if (sections.length) {
      status.textContent = songs.length
        ? '已读取全部 ' + songs.length + ' трекаЕжедневные рекомендации；滚动时仅渲染视窗附近歌曲'
        : '来自网易云推荐歌单';
      ...
      list.innerHTML = homePlatformRecommendationEmptyHtml('netease', homeDiscoverState.loggedIn
        ...
        : '登录网易云后可读取推荐歌单与Ежедневные рекомендации，未使用关键Текст搜索替代。');
    }
    return;
  }
  ...
  if (feedConfig && feedState) {
    ...
    if (feedState.loading) {
      status.textContent = '正在读取' + sourceLabel + 'Выбор платформ…';
      list.innerHTML = '<div class="home-platform-recommend-loading">正在同步推荐内容</div>';
      return;
    }
    if (feedState.songs.length) {
      ...
      if (source === 'qishui' && feedState.fallback) {
        sectionTitle = '你的音乐';
        cardLabel = '汽水喜欢 / Недавно воспроизведённые';
        readyText = '汽水推荐 Feed 暂不可用，当前显示你的喜欢与Недавно воспроизведённые';
      ...
      list.innerHTML = homePlatformRecommendationEmptyHtml(source, feedState.message || (feedFailed
        ? '推荐接口当前不可用，未使用关键Текст搜索补位。'
        : '连接' + sourceLabel + '后可读取Выбор платформ，未使用关键Текст搜索替代。'));
    }
    return;
  }

  status.textContent = homePlatformRecommendationSourceLabel(source) + ' 暂无Выбор платформ接口';
