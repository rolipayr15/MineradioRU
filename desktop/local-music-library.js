...
class LocalMusicLibrary {
    ...
    try {
      ...
      for (const source of parsed.records.slice(0, MAX_IMPORT_FILES)) {
        ...
        const record = {
          ...
          artist: cleanText(source.artist, 'Локальный Файл', 1000),
  ...
  serializeRecord(record) {
    ...
    return {
      ...
      name: record.name,
      title: record.name,
      artist: record.artist || 'Локальный Файл',
      album: record.album || '',
  ...
  async parseEntry(entry, sidecarDirectories) {
    ...
    return {
      record: {
        ...
        artist: cleanText(common.artist || artists, metadataError && previous ? previous.artist : 'Локально文件', 1000),
