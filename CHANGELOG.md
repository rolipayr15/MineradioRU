...
- 修复 QQ 会员状态识别与登录同步。
- 修复无可用音源时反复Режим воспроизведения导致的卡顿与闪退。
...
- 重要安全说明：`v1.0.10` 及更早旧安装包不再建议继续安装或传播，请先隔离旧安装包；本次不提供从 `v1.0.10` 到 `v1.1.0` 的软件内Локально更新，请到 GitHub Release 手动下载 `v1.1.0` 进行纯净安装。
- 默认视觉参数更新：将 `默认测试.json` 作为трека次启动默认用户存档和软件内视觉参数默认值；新用户трека次启动即使用同一份快照，用户存档中自动预置「默认测试」。
- 默认视觉快照包括：`emily` 视觉预设、封面粒子分辨率 `1.55`、电影镜头强度 `0.5`、歌Текст发光 `0.28`、高亮色 `#fac900`、发光色 `#008aff`、UI/Home/视觉图标白色、3D 歌单架静态常驻、播客歌单默认关闭、侧向角度 `-15`。
- 3D 歌单架新增内容开关：可关闭播客歌单，可Выбрать是否把「我的歌单」与「收藏歌单」合并成一条连续滚动列表。
- 3D 歌单架交互修复：动态/静态详情页绑定分流，静态详情页跟随封面粒子世界轴，动态详情页跟随镜头；常驻歌单架未命中时不再长期挡歌Текст，命中卡片时才浮起到前景。
- 3D 歌单详情页修复：详情页打开后保持更完整的页面显示和中心高亮行；修复固定/常驻状态下详情页被歌Текст或卡片遮挡、动态镜头触发区异常、滚轮高亮不同步等问题。
- 歌Текст透明度修复：详情页选歌/切歌时新Текст不再突然跳亮；详情页打开时歌Текст保持可读，但降低发光层并保持在详情页下层，避免遮挡中心高亮行。
...
- 更新下载会复用Локально已校验通过的安装包，避免重复下载同一文件。

## v1.0.10

- 重做桌面歌Текст视觉与交互：保持软件内歌Текст质感，优化白底可读性、长歌Текст滚动、锁定穿透、电影震动与最小化状态一致性。
- 修复安魂预设歌Текст位置无法调整的问题，当前嘴部歌Текст原位置作为原点，景深改为按视角前后移动。
- 歌Текст布局新增上下角度与左右角度调节，并会随Локально布局和用户存档保存。
- 歌Текст镜头绑定开启时避让右侧 3D 歌单架，安魂预设也同步处理，避免遮挡歌单架操作。
...
- 安装包支持用户自由Выбрать安装目录；默认仍优先 `D:\Mineradio`，Выбрать盘根目录时会自动补全 `Mineradio` 文件夹。
...
- Home 左侧大卡片改为施工提示，并支持在 Home 页手动Развернуть панель управления。
- 视觉预设顺序调整为 emily、安魂、星河、唱片、星球、滚筒、虚空；安魂十字架选中态保持同款蓝色。
- 视觉控制台新增 4 个槽位的“用户存档”，可保存粒子、颜色、滑条和歌Текст外观习惯，并支持卡片内直接命名。
- 修复重启后歌Текст预设/自定义颜色没有立即恢复到舞台歌Текст的问题。
...
- 优化 GitHub 更新下载：默认优先尝试国内加速线路，失败后自动Режим воспроизведения下一条线路，最后兜底 GitHub 直连。
...
- 重构视觉预设控制台的信息结构，新增 `预设 / 外观 / 歌Текст / 动态 / 高级` 五个分区，减少功能堆叠带来的操作压力。
...
- 修复网易云 / QQ 扫码成功但登录态不落地的问题：网易云不再因资料接口短暂无 profile 立刻Очистить cookie，QQ 登录支持新式 `psrf_*` / `wx*` token。
- 修复播放/暂停按钮手动恢复播放失效：手动点击路径现在会在用户点击同步栈内先调用 `audio.play()`，再恢复音频分析器，避免错过浏览器用户激活窗口。
- 修复播放准备阶段的栈溢出会拖死控制台的问题：歌Текст预置、封面加载、喜欢状态、Home 状态、listen session 和 beatmap / DJ 视觉预热均改为隔离执行，任何非音频链路异常不再阻断真实切歌。
...
- 修复天气电台 / 歌单播放链路的控制台递归风险：不再运行时重写播放、上一трека、下一трека按钮的 `onclick`，避免 `Maximum call stack size exceeded`。
...
- 天气电台、歌单、播客、Ежедневные рекомендации和 3D 详情播放入口统一使用安全队列刷新；队列写入后播放器控制台会恢复可点击，不再被 UI 渲染异常拖死。
- 恢复歌单 / 天气电台自动开播为非 manual 路径，并将 `playQueueAt` 前置 UI 阶段纳入兜底，避免队列标题已变但实际音频和上下一трека按钮停在旧状态。
- 天气电台音乐来源收窄为网易云搜索，并取消入队前批量播放 URL 探测，避免点击入口时请求堆积造成卡死。
- 天气电台前端复用正在进行的天气加载请求，避免 Home 后台预加载和手动启动同时打到后端。
- 自动播放被系统拦截时，播放器控制台保持可点击并提示手动继续；真正加载失败的队列项会自动跳到下一трека。
- 手动下一трека / 上一трека现在一定Режим воспроизведения实际音频；单曲循环只在自然播完时重复当前曲。
- 切歌开始时立即暂停旧音频，避免出现歌Текст和 UI 已切走但旧歌还在播的错位。
...
- 启动页退出后补 Home 恢复安全网，避免трека页隐藏在空场背景后。

- 发布 Mineradio трека个正式版本，GitHub 主页和安装包版本统一为 `1.0.0`。
- трека页正式版包含天气电台、Ежедневные рекомендации、私人电台、继续听、听歌画像和我的歌单入口。
- 天气电台接入 Open-Meteo，并优先混入Ежедневные рекомендации和私人推荐曲库，过滤 AI / 白噪音 / 雨声助眠等低质结果。
- 修复窗口模式下主页 6 张主卡片在紧凑尺寸中过早堆叠、挤出трека屏的问题。
- 保持启动后 Wallpaper 银河背景干净显示，播放后Режим воспроизведения到 Emily / 默认播放态视觉。
...
- 优化电影镜头节奏分析与 DJ 视觉模式
- 增加自定义封面、自定义歌Текст、歌Текст布局和视觉控制相关能力

## 说明

这是трека次公开发布前的整理版本。更早版本属于Локально迭代记录，暂不展开。
