...
function playbackFailureToastText(err) {
  ...
  if (/notsupportederror|no supported source|decode|media_err_decode/.test(lower)) return '播放失败：音频格式或解码失败，建议Изменить源或降低音质';
  ...
  if (/copyright|unavailable|not playable|url.*empty|no url/.test(lower)) return '播放失败：平台没有返回可播放地址，建议Изменить源';
  return '播放失败：' + (msg || '未知原因，请尝试Изменить源或重新登录');
}
